...
CrashCymbal::CrashCymbal(const Vector& position)  : GameObject(position)
{
	//generate vertices
...
void CrashCymbal::update(double elapsedTime)
{
	
}

void CrashCymbal::render()
{
	glPushMatrix();
...
bool CrashCymbal::collision(const Vector& otherObject)
{
	//double dist = m_Position.distance(otherObject);
	return m_Position.distance(otherObject) < .3;
}

void CrashCymbal::destroy()
{
