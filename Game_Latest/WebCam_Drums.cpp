...
void Render()
{
	...
	p->render();
	//c.render();
	//counter = c.get_position().distance(left);
	//std::string result;
	//std::ostringstream convert;
	//convert << counter;
	//result += convert.str();
	//f->set_text(std::string("Distance: ") + result);
	//f->render();
	//counter++;
	glFlush();
	glutSwapBuffers();
...
int _tmain(int argc, char** argv)
{
	...
	//left.setY(-.0371);
	InitGL();
	f = new Font("Distance: ", std::wstring(L"Arial Black"), Vector(0, 0, -.1f), Vector(0, 0, 1.0f), -36); 
	p = new PlayField((tan(.5 * FIELD_OF_VIEW) * 2 * NEAR_CLIPPING_PLANE) - .06, tan(.5 * FIELD_OF_VIEW) * 2 * NEAR_CLIPPING_PLANE * (SCREEN_HEIGHT  / SCREEN_WIDTH), .1, .06, "Good_MCHammer.gh");
	glutMainLoop();
	return 0;
}
